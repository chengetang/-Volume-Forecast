...
def _load_route_mapping(mapping_rows):
    """把 config_data_test.py 里的 ROUTE_MAPPING_DATA（(路线号, 邮编) 列表）转成 邮编 -> 路线号 的查找表。
    数据直接写在 config 文件里，而不是从 data/*.xlsx 读取，是因为部分同事用 launcher.py
    启动程序，它只会从 GitHub 拉取 config_data.py / app.py 这两个文件，不会拉取 data/ 目录，
    放进 config 文件才能保证路线号映射也能跟着这两个文件一起分发到每个人的电脑上。
...
ROUTE_MAPPING = _load_route_mapping(ROUTE_MAPPING_DATA)
